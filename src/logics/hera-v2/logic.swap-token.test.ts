...
import { getHeraRouterAddress } from './configs';
import { metisTokens } from './tokens';

describe('HeraV2 SwapTokenLogic', function () {
  ...
  context('Test build', function () {
    const testCases: LogicTestCaseWithChainId<SwapTokenLogicFields, SwapTokenLogicOptions>[] = [
      ...
      },
      {
        ...
        fields: {
          ...
          output: new common.TokenAmount(metisTokens.DAI, '0'),
          slippage: 100,
        },
        options: { account: '0xaAaAaAaaAaAaAaaAaAAAAAAAAaaaAaAaAaaAaaAa' },
      },
    ...
    testCases.forEach(({ chainId, fields, options }) => {
      it(`${fields.input.token.symbol} to ${fields.output.token.symbol}`, async function () {
        ...
        const { input } = fields;
        const routerAddress = getHeraRouterAddress(chainId);
        ...
        expect(routerLogic.approveTo).to.eq(routerAddress);
